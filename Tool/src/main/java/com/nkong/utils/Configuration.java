...
public class Configuration {
	...
	public static void main(String[] args) {
		Configuration from = from("{'a1':[{'a12':'123'}], 'a2':'456'}");
        Object a3 = from.set("a3", "789");
    ...
    /**
     * 根据用户提供的json path，寻址Character对象，如果对象不存在，返回默认Character对象
     *
    ...
    private Object findObjectInMap(final Object target, final String index) {
        ...
        if (null == result) {
            throw new IllegalArgumentException(String.format(
                    "您提供的配置文件有误. 路径[%s]值为null，程序无法识别该配置. 请检查您的配置并作出修改.", index));
        }
    ...
    private Object findObjectInList(final Object target, final String each) {
        ...
        if (!StringUtils.isNumeric(index)) {
            throw new IllegalArgumentException(
                    String.format(
                            "系统编程错误，列表下标必须为数字类型，但该节点发现实际类型是[%s] ，该异常代表系统编程错误, 请联系程序开发团队 !",
                            index));
        }
    ...
    private static void checkJSON(final String json) {
        if (StringUtils.isBlank(json)) {
            throw new NullPointerException("您输入的Json为空");
        }
    }
